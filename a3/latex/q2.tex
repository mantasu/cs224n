...
\begin{parts}
    ...
    \ifans{A sentence of $n$ words will be parsed in $2n$ steps because the dependency parser is linear in time. In other words, it works with pairs of words and it does not support nested dependencies thus it will take $n$ steps for all the \texttt{SHIFT} operations and another $n$ steps for \texttt{LEFT-ARC} and \texttt{RIGHT-ARC} operations. 
    }\newline
